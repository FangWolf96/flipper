...
If (Test-Path $LOGMEIN_INSTALLATION_FILE)
{
    ...
    Start-Process -FilePath "msiexec" -ArgumentList "/i $LOGMEIN_INSTALLATION_FILE /qn"

    AddLog "Finished"
}
Else
{
    AddLog "[Adobe Acrobat] Cannot install. NOT found installation file $LOGMEIN_INSTALLATION_FILE"
}
